...
export async function consolidateInmediate(symptoms: Array<string>): Promise<object> {
  try {
    ...
    if(!sharedData.appopintmentReservedId) return new Error('For consolidate a new appointment you must provide an appointment id, throught reserve method, or define in sharedData class')
